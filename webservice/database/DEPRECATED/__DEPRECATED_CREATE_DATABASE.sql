...
CREATE  TABLE IF NOT EXISTS `tcc_gfx`.`residencias` (
  ...
  INDEX `FK_belongs_to` (`user_id` ASC) ,
  INDEX `FK_distribuidora_idx` (`distribuidora_id` ASC) ,
  CONSTRAINT `FK_owner`
    FOREIGN KEY (`user_id` )
...
CREATE  TABLE IF NOT EXISTS `tcc_gfx`.`medidas` (
  ...
  INDEX `FK_measured_at` (`house_id` ASC) ,
  CONSTRAINT `FK_medido_at`
...
CREATE  TABLE IF NOT EXISTS `tcc_gfx`.`medidas_dia` (
  ...
  INDEX `FK_measured_at` (`house_id` ASC) ,
  CONSTRAINT `FK_collected_at`
    FOREIGN KEY (`house_id` )
    REFERENCES `tcc_gfx`.`medidas` (`house_id` )
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE  TABLE IF NOT EXISTS `tcc_gfx`.`medidas_mes` (
  ...
  INDEX `FK_measured_at` (`house_id` ASC) ,
  CONSTRAINT `FK_gathered_at`
    FOREIGN KEY (`house_id` )
    REFERENCES `tcc_gfx`.`medidas_dia` (`house_id` )
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
